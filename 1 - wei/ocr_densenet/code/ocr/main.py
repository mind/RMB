...
def test(epoch, model, train_loader, phase='test'):
    ...
    for i,data in enumerate(tqdm(train_loader)):
        ...
        old_preds = probs.data.cpu().numpy().max(1)
        preds_all.append(old_preds)

        preds = probs.data.cpu().numpy()>0.5 # (-1, 8, 1824)
        # np.save(os.path.join(feat_dir, name.replace('.png','_preds.npy')), preds)
    ...
    np.save('../../data/result/{:d}_{:s}_preds_max1.npy'.format(epoch, phase), preds_all)
...
def main():
    ...
    # 划分数据集
    test_filelist = sorted(glob(os.path.join('/content/test_cptn_result','*')))
    # test_filelist = sorted(glob(os.path.join('/content/train_cptn_result','*')))

    trainval_filelist = sorted(glob(os.path.join('/content/test_cptn_result','*')))
    ...
    train_filelist1 = train_filelist1[:]
    print('len(train_filelist1)',len(train_filelist1))
    train_filelist2 = train_filelist2
    print('len(train_filelist2)',len(train_filelist2))
